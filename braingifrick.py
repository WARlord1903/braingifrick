...
from _brainfrick.lib import interpret_code
...
def image_to_ascii(image_path, new_width=100, arr=False):
    if isinstance(image_path, str) and image_path[-4:] in ['.mp4', '.avi', '.mov', '.mkv', '.gif']:
        ...
        ascii_image = ""
        for pixel in pixels:
            ascii_image += ascii_luminosity_chars[nearest_match(pixel / 255, ascii_luminosity_vals)]
        
        ascii_image = "\n".join(ascii_image[i:(i+new_width)] 
...
interpret_code(b'', True, False)
im = ascii_to_brainfrick(image_to_ascii('Motor.png'))
if isinstance(im[0], list):
    for frame in im:
        for line in frame:
            interpret_code(line.encode('ascii'), False, False)
        interpret_code(b'', True, False)
        sleep(1 / 60)
        os.system('clear')
else:
    for line in ascii_to_brainfrick(image_to_ascii('badapple.mp4')):
        interpret_code(line.encode('ascii'), False, False)
interpret_code(b'', False, True)